...
The runtime now has explicit coverage for the decision priority between â€œclarifyâ€? â€œfixâ€? â€œwrite backâ€? â€œcontinueâ€? and â€œstopâ€? instead of leaving next-step decisions to informal interpretation of multiple upstream statuses.

## Facts / Skills Updated
- no new facts were written
- no new skills were created
- updated stable runtime docs in `README.md` and `ai/doc/runtime/*` to reflect the implemented follow-up suggester
