...
## Proposed Approach
Add a small runtime design package under `ai/doc/runtime/`, minimally refine starter files that are inaccurate for this project, and implement a stdlib-only Python CLI around a single real node, `plan-to-spec`. Dogfood the runtime by compiling this roadmap into the current implementation spec in `ai/doc/specs/`.
...
- dogfood the roadmap into a real task spec in `ai/doc/specs/`
- add black-box and white-box `unittest` coverage for the slice

### Done When
A contributor can use `ae-runtime plan-to-spec --plan ai/doc/runtime/roadmap.md` to compile this roadmap into a draft task spec and JSON run log, and the repository clearly reads as a lightweight runtime layered on top of the copied SOP starter.

### Black-box Checks
- compile `ai/doc/runtime/roadmap.md` into the next dated task-spec path
...
### Write-back Needed
Yes - keep `ai/doc/facts/project-scope.md` aligned with the stable current phase and boundary language clarified by this slice.
