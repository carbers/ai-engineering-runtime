...
## Done When
A contributor can run `python -m ai_engineering_runtime run-history-select --spec ai/doc/specs/20260322-005-validation-collect-foundation.md --node validation-collect --limit 3` and receive the newest relevant replayable runs for that exact artifact target, or a stable no-match result, without the runtime introducing fuzzy correlation or a separate history store.
...
## Write-back Needed
Yes - update `ai/doc/runtime/protocol.md` and `README.md` only if the selector contract and CLI surface stabilize during implementation.
