...
class WritebackPackageNode:
    name = "writeback-package"

    def __init__(self, request: WritebackPackageRequest):
        self.request = request

    def execute(self, adapter: FileSystemAdapter) -> RunResult:
        package, reasons = resolve_writeback_package_query(
            adapter,
        ...
        result = RunResult(
            ...
            to_state=state,
            issues=reasons,
            writeback_package=package,
            output_path=adapter.build_writeback_package_path(package.run_id) if package is not None else None,
